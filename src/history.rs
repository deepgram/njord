...
impl History {
    ...
    pub fn rename_session_with_source(&mut self, old_name: &str, new_name: &str, source: crate::session::NameSource) -> Result<bool> {
        // Check if old session exists
        if !self.saved_sessions.contains_key(old_name) {
            return Ok(false);
        }
    ...
    pub fn get_sessions_for_auto_rename(&self) -> Vec<(&String, &ChatSession)> {
        self.saved_sessions
            .iter()
            .filter(|(_, session)| {
                // Include sessions with no name_source (legacy) or timestamp-based names
                match &session.name_source {
                    None => true, // Legacy sessions without name_source tracking
                    Some(crate::session::NameSource::Timestamp) => true,
                    Some(crate::session::NameSource::UserProvided) => false,
                    Some(crate::session::NameSource::AutoGenerated) => false,
                }
            })
            .collect()
    }
