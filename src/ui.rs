...
impl NjordCompleter {
    ...
    fn complete_chat_command(&self, line: &str, pos: usize) -> Vec<Pair> {
        ...
        if parts.len() == 1 || (parts.len() == 2 && !input.ends_with(' ')) {
            ...
            if matches!(
                subcommand,
                "load" | "delete" | "continue" | "merge" | "auto-rename"
            ) {
    ...
    fn complete_session_names(&self, current_word: &str) -> Vec<Pair> {
        ...
        let is_quoted_input =
            current_word.trim().starts_with('"') || current_word.trim().starts_with('\'');
    ...
    fn complete_session_references(&self, current_word: &str) -> Vec<Pair> {
        ...
        if let Some(_number_part) = current_word.strip_prefix('#') {
            for i in 1..=10 {
                // Show up to 10 ephemeral references
    ...
    fn complete_prompts_command(&self, line: &str, pos: usize) -> Vec<Pair> {
        ...
        if parts.len() == 1 || (parts.len() == 2 && !input.ends_with(' ')) {
            ...
            if matches!(
                subcommand,
                "show" | "apply" | "delete" | "edit" | "auto-name"
            ) {
    ...
    fn find_longest_common_prefix(&self, completions: &[Pair], current_word: &str) -> String {
        ...
        if !current_is_quoted && completions_are_quoted {
            ...
            let unquoted_completions: Vec<String> = completions
                .iter()
                .map(|c| {
            ...
            if let Some(first_unquoted) = unquoted_completions.first() {
                ...
                for completion in &unquoted_completions[1..] {
                    ...
                    for (i, (&p_char, &c_char)) in
                        prefix_chars.iter().zip(completion_chars.iter()).enumerate()
                    {
                        if p_char == c_char {
                            common_len = i + 1;
                        } else {
        ...
        let completion_texts: Vec<&str> = completions
            .iter()
        ...
        if parts.len() == 1 || (parts.len() == 2 && !input.ends_with(' ')) {
            ...
            match setting {
                ...
                "thinking" => {
                    ...
                    let values = [
                        "true", "false", "on", "off", "enabled", "disabled", "yes", "no",
                    ];
                    return values
                        .iter()
                ...
                "temperature" => {
                    ...
                    let values = [
                        "0.0", "0.1", "0.3", "0.5", "0.7", "0.8", "1.0", "1.2", "1.5", "2.0",
                    ];
                    return values
                        .iter()
                        .filter(|value| value.starts_with(current_word))
    ...
    fn hint(&self, line: &str, pos: usize, _ctx: &rustyline::Context<'_>) -> Option<Self::Hint> {
        ...
        match completions.len() {
            ...
            n if n > 1 => {
                ...
                let completion_names: Vec<String> = completions
                    .iter()
...
impl UI {
    ...
    pub fn read_input(
        &mut self,
        prompt_info: Option<(&str, &str)>,
        session_name: Option<&str>,
        is_anonymous: bool,
        ephemeral: bool,
        user_msg_number: Option<usize>,
        ...
        let (prompt, initial_input) = if let Some((message, status)) = prompt_info {
            let color = match status {
                ...
                } // Yellow for ephemeral, green default
            };
            ...
            (
                format!(
                    "{}{}{}>>> ({}) \x1b[0m",
                    color, session_prefix, user_prefix, status
                ),
                message,
            )
        } else {
        ...
        match self
            .editor
            .readline_with_initial(&prompt, (initial_input, ""))
        {
            Ok(line) => {
                let input = line.trim();
    ...
    fn read_multiline_input(
        ...
        if let Some(ref _tag_name) = tag {
            println!(
                "\x1b[2m(Multi-line mode - end with '{}' on its own line)\x1b[0m",
                end_marker
            );
        } else {
        ...
        loop {
            match self.editor.readline(&format!(
                "{}{}{}... \x1b[0m",
                color, session_prefix, user_prefix
            )) {
                Ok(line) => {
    ...
    fn read_heredoc_input(
        ...
        // Show helpful message about the expected end marker
        println!(
            "\x1b[2m(Heredoc mode - end with '{}' on its own line)\x1b[0m",
            delimiter
        );
    ...
    fn handle_multiline_history_entry(
        &mut self,
        multiline_input: &str,
        _prompt: &str,
        session_name: Option<&str>,
    ...
    fn edit_multiline_content(
        ...
        println!(
            "\x1b[2mType your content. End with a line containing only '}}' to finish.\x1b[0m"
        );
        println!();
    ...
    pub fn parse_command_heredoc(&self, line: &str) -> Option<(String, String)> {
        ...
        if let Some(heredoc_pos) = trimmed.find("<<") {
            ...
            if !command_part.is_empty()
                && !delimiter_part.is_empty()
                && !delimiter_part.contains(char::is_whitespace)
            {
        ...
        let handle = tokio::spawn(async move {
            ...
            while spinner_active_clone.load(Ordering::Relaxed) {
                ...
                if spinner_active_clone.load(Ordering::Relaxed) {
                    print!(
                        "\r\x1b[2K\x1b[1;33m{} {}\x1b[0m",
                        spinner_chars[frame], message
                    );
                    io::stdout().flush().unwrap();
                }
            }
