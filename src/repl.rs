        ...
        loop {
            ...
            let (session_name, is_anonymous) = if self.session.messages.is_empty() {
                ...
                if let Some(name) = &self.session.name {
                    // Check if this is an anonymous session based on name_source
                    let is_anonymous_session = match &self.session.name_source {
                        ...
                        None => {
                            // Legacy sessions without name_source - check if name looks like timestamp
                            name.chars().any(|c| c == '_' && c.is_ascii_digit()) && 
                            name.len() >= 19 // YYYY-MM-DD_HH:MM:SS format
                        }
                    };
                    (Some(name.as_str()), is_anonymous_session)
                } else {
