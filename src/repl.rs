...
impl Repl {
    ...
    async fn handle_auto_rename(&mut self, session_name: Option<String>) -> Result<()> {
        ...
        match self.history.rename_session_with_source(&current_name, &sanitized_title, crate::session::NameSource::AutoGenerated) {
            Ok(true) => {
        ...
        for (session_name, session) in candidates {
            ...
            match self.generate_session_title(&first_user_message.unwrap().message.content).await {
                Ok(generated_title) => {
                    ...
                    if self.history.load_session(&sanitized_title).is_some() {
                        self.ui.print_error(&format!("Cannot rename \"{}\" to \"{}\" - name already exists", session_name, sanitized_title));
                        failed_count += 1;
                        continue;
                    }
                    
                    // Rename the session
                    match self.history.rename_session_with_source(session_name, &sanitized_title, crate::session::NameSource::AutoGenerated) {
                        Ok(true) => {
                            self.ui.print_info(&format!("\"{}\" → \"{}\"", session_name, sanitized_title));
                            renamed_count += 1;
